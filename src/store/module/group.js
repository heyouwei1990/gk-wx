    ...
    actions: {
            ...
            let options = {
                ...
                success: function(resp) {
                    ...
                    GetJobGroupInfo({
                        ...
                        if (r.Result == 1) {
                            let list = r.Data.Members;
                            for (let i = 0; i < list.length; i++) {
                                for (let j = 0; j < members.length; j++) {
                                    if (members[j].member == list[i].EasemobUserName || members[j].owner == list[i].EasemobUserName) {
                                        Object.assign(members[j], list[i]);
                                    }
                                }
                            }
                        } else {
        ...
        onCreateGroup: function(context, payload) {
            ...
            let options = {
                ...
                success: function(resp) {
                    context.dispatch('onGetGroupUserList')
                },
                error: function() {}
            };
        ...
        onInviteGroup: function(context, payload) {
            ...
            let option = {
                ...
                success: function(res) {
                    Toast.success("邀请成功，等待用户同意")
                },
                error: function(err) {
                    if (JSON.parse(err.data).error == "forbidden_op") {
                        Toast.fail(JSON.parse(err.data).error_description)
                    }
                }
            };
        ...
        onUpdataGroupInfo: function(context, payload) {
            ...
            let option = {
                ...
                success: function(resp) {
                    context.dispatch('onGetGroupUserList')
                    context.dispatch('onGetGroupinfo', { select_id })
                    this.$forceUpdate();
                }
            };
