...
export default {
  ...
  methods: {
    pageInit(){
      ...
      //赋值
      let query=JSON.parse(this.$route.query.params);
      let specialRemarkList=query.specialList || [];
      for (let i = 0; i < this.specialList.length; i++) {
        for (let j = 0; j < specialRemarkList.length; j++) {
          if(specialRemarkList[j].RemarkType==this.specialList[i].RemarkType){
            this.specialList[i].SelectStatus=specialRemarkList[j].SelectStatus;
            this.specialList[i].Remark=specialRemarkList[j].Remark;
          }
        }
      }
    },
    ...
    submit(){
      ...
      AddJobDetail(params).then(r=>{
        if(r.Result==1){
          let data=r.Data;
          this.$router.push({path:"/createOrder3",query:{jobId:data.JobId,isProject:data.IsB2BProjectJob,jobType:params.JobType,jobTitle:params.orderName}});
        }else{
