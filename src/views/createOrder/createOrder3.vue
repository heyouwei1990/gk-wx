<template>
  <div class="createOrder">
    ...
    <div class="form-item" v-if="showItems.IsJobAddress !=0 ">
      ...
      <span class="btn-address" @click="$router.push('/addressList')">地址簿</span>
    ...
    <div class="divide"></div>
    <div class="form-item" v-if="showItems.IsSecondLinkMan!=0">
      <div class="label">现场备用联系人<i class="required" v-if="showItems.IsSecondLinkMan==1">*</i></div>
      <input class="input" v-model.trim="form.SecondLinkMan" type="text" placeholder="现场备用联系人" />
    </div>
    <div class="form-item no-border" v-if="showItems.IsSecondHandSet!=0">
      <div class="label">联系电话<i class="required" v-if="showItems.IsSecondHandSet==1">*</i></div>
      <input class="input" v-model.trim="form.SecondHandSet" type="tel" placeholder="备用联系人电话" />
    </div>
    <div class="divide"></div>
    ...
    <div class="form-item" v-if="(showItems.IsBudgetRanger !=0 && !isProject) || isProject">
      <div class="label">预算总价<i class="required" v-if="(showItems.IsBudgetRanger ==1  && !isProject) || isProject">*</i></div>
      <input class="input" v-model.number="form.MinPrice" type="number" placeholder="最低预算" />
      <span class="line">至</span>
      <input class="input" v-model.number="form.MaxPrice" type="number" placeholder="最高预算" />
    </div>
    ...
    <div class="divide"></div>
    <div class="urgent-item" v-if="jobType==1">
      <div class="flex">
    ...
    <!-- <div class="protocol-wrapper">
      <van-checkbox shape="square" checked-color="#00A0E9" v-model.trim="form.IsAgree">
        <span class="agree">同意</span>
        <span class="protocol" @click.stop="$router.push('/createOrderProtocol')">《工业速派在线技术服务发布规则》</span>
      </van-checkbox>
    </div> -->
    <div class="btn-box">
    ...
    <van-popup class="deposit-dialog" v-model="showDeposit" position="bottom">
      <template v-if="depositData">
        ...
        <div class="btn-group">
          <div class="btn-primary" @click="$router.push({path:'/payDeposit',query:{payId,jobType,jobId}})">确定</div>
        </div>
      </template>
    </van-popup>
  </div>
...
export default {
  ...
  watch: {
    $route(to,from){
      ...
      if(from.path=='/addressList'){
        if(this.pickedAddress){
          this.form.Lat=this.pickedAddress.Lat;
          this.form.Lng=this.pickedAddress.Lng;
          this.form.CityName=this.pickedAddress.CityName;
          this.form.FullAddress=this.pickedAddress.FullAddress;
          this.form.AddressID=this.pickedAddress.ID;
          this.form.LinkMan=this.pickedAddress.ContactsName;
          this.form.HandSet=this.pickedAddress.ContactsPhone;
          this.form.Address=this.pickedAddress.Address;
        }
      }
    },
  },
  ...
  activated() {
    this.jobType=Number(this.$route.query.jobType);
      console.log(Number(this.$route.query.jobType))
  ...
  methods: {
    ...
    //再发一单
    createAgain(){
      let step2=this.$store.state.publishAgain;
      if(step2){
        //form对象
        for (let k in step2) {
        ...
        this.form.MinPrice=Number(step2.MinPrice)?Number(step2.MinPrice):'';
        this.form.MaxPrice=Number(step2.MaxPrice)?Number(step2.MaxPrice):'';
        this.form.ServiceDayCount=step2.ServiceDayCount?step2.ServiceDayCount:'';
        this.extraItems=step2.ExtendValueList || [];
      }
    },
    //获取代发工单信息
    getAgencyData(){
      GetReplaceSendJob({
        ...
        if(r.Result==1 && r.Data.ReplaceSendJobId){
          let step2=r.Data.SecondStep
          //form对象
          for (let k in step2) {
            if (Object.prototype.hasOwnProperty.call(this.form,k)) {
              this.form[k]=step2[k];
    ...
    pickAddress(item){
      ...
      this.form.FullAddress=item.FullAddress;
      this.form.LinkMan=item.ContactsName;
      this.form.HandSet=item.ContacesPhone;
    ...
    next() {
      ...
      AddJobBase(params).then(r=>{
        if(r.Result==1){
          this.$toast.success('发单成功');
          let data=r.Data;
          if(data.JobType==8){
            this.$router.push({path:'/orderProcess',query:{jobId:this.jobId,jobType:8}});
          }else if(data.JobType==6 && !data.IsProjectJob){
            //B2B标准工单去支付页面
            this.getDeposit();
            this.payId=data.PayId;
            // this.$router.push({path:'/payDeposit',query:{payId:data.PayId}});
          }else if(data.JobType==1 && data.IsUrgent){
            this.$router.push({path:'/payDeposit',query:{jobId:this.jobId,payId:data.PayId,from:3}})
          }else{
            this.$router.push({path:'/createSuccess',query:{jobId:this.jobId,jobType:data.JobType}})
          }
          this.$store.commit('setProductPicker',null);
          this.$store.commit('setPublishAgain',null);
        }else{
          this.$toast(r.Message);
        }
      })
